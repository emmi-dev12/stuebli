...
Future expansion may include printable gadgets, custom objects, and broader home modeling. The initial implementation uses SwiftUI/AppKit, SceneKit, and a renderer-independent Swift scene module with a CLI. Stack selection was delegated by the instruction to proceed and build. Realistic rendering remains an unfulfilled quality target.

## Platform

Native macOS (initially Apple Silicon tested).

## Current catalog

Twelve IKEA Switzerland product variants are sourced in catalog/ikea-ch. Product dimensions are verified; procedural visual models are approximate.
